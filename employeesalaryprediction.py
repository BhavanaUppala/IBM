...
def generate_sample_data():
    ...
    for i in range(n_samples):
        ...
        salary = base_salary * \
            ...
            locations[location] * company_sizes[company_size]
...
class DataPreprocessor:
    ...
    def fit_transform(self, df):
        ...
        categorical_columns = ['Job_Title',
                               'Education_Level', 'Location', 'Company_Size']

        for col in categorical_columns:
            le = LabelEncoder()
            df_processed[col +
                         '_Encoded'] = le.fit_transform(df_processed[col])
            self.label_encoders[col] = le

        self.feature_columns = ['Years_Experience'] + \
...
def create_visualizations(df, results):
    ...
    plt.subplot(2, 2, 1)
    plt.hist(df['Salary'], bins=30, alpha=0.7,
    ...
    plt.plot(experience_groups.index, experience_groups.values,
    ...
    plt.subplot(2, 2, 3)
    job_salary = df.groupby('Job_Title')[
    ...
    edu_salary = df.groupby('Education_Level')[
        'Salary'].mean().reindex(edu_order)
    ...
    bars = plt.bar(model_names, r2_scores, color=[
    ...
    numeric_cols = ['Years_Experience', 'Salary'] + \
        [col + '_Numeric' for col in categorical_cols]
    ...
    if st.session_state.predictor is not None:
        ...
        if st.button("🔮 Predict Salary", type="primary"):
            try:
                ...
                st.info(
                    f"🎯 **Prediction Range:** ${lower_bound:,.0f} - ${upper_bound:,.0f}")
        ...
        if 'results' in st.session_state:
            ...
            st.success(
                f"🏆 **Best Model:** {best_model_name} with R² Score of {best_r2:.3f}")
        ...
        st.info(
            "👆 Please train the model using the sidebar to start making predictions!")
